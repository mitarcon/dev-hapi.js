  ...
  async register (server, options) {
    ...
    server.route({
      ...
      handler: async (req, h) => {
        let result
        try {
          result = await QuestionModel.getOne(req.params.key)
          if (!result) {
    ...
    server.route({
      ...
      handler: async (req, h) => {
        ...
        let result
        try {
          result = await QuestionModel.getLast({ amount: req.params.amount })
          if (!result) {
    ...
    async function validateAuth (req, username, passwd, h) {
      let user
      try {
        user = await UserModel.login({ email: username, passwdord: passwd })
      } catch (error) {
